...
class AbstractManager:
    ...
    def check_is_equal(
        first_element_name, first_element, second_element_name, second_element
    ):
        if first_element != second_element:
            raise ValidationError(
                {
                    "error": f"{first_element_name.__class__.__name__.lower()} and {second_element_name.__class__.__name__.lower()} are not linked."
                }
            )
    ...
    def check_if_db_element_with_name_exist(className, name: str):
        if className.objects.filter(name=name).exists():
            raise ValidationError(
                {
                    "error": f"{className.__name__.lower()} with this name already exists."
                }
            )
    ...
    def check_if_db_element_with_email_exist(className, email: str):
        if className.objects.filter(email=email).exists():
            raise ValidationError(
                {
                    "error": f"{className.__name__.lower()} with this email already exists."
                }
            )
    ...
    def check_valid_field_in_kwargs(className, **kwargs):
        ...
        for key in kwargs.keys():
            if key not in valid_fields:
                raise ValidationError(
                    {
                        "error": f"{key} is not a valid field of {className.__name__.lower()}."
                    }
                )
    ...
    def check_valid_shift_interval(start_time, end_time, user_id):
        ...
        for shift in ShiftRepository.get_shifts_by_user_id(user_id):
            if start_time < shift.end_time and end_time > shift.start_time:
                raise ValidationError(
                    {"error": "Shift time interval overlaps with an existing shift."}
                )
