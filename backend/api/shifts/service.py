...
class ShiftManager(AbstractManager):
    ...
    @staticmethod
    def list_shifts_by_team_id(team_id: int, from_date=None, to_date=None):
        return ShiftRepository.get_shifts_by_team_id(
            team_id=team_id, from_date=from_date, to_date=to_date
        )

    @staticmethod
    def create_shift(user: Users, start_time: datetime, end_time: datetime):
        return ShiftRepository.create_shift(
    ...
    def update_shift(shift_id: int, start_time: datetime, end_time: datetime):
        return ShiftRepository.update_shift(
            shift_id=shift_id, start_time=start_time, end_time=end_time
        )
